...
| `LILO_DATABASE_DOCKER_PORT` | u16 port | `56432` | `compose.yaml` (Compose host port only; not read by `lilo`) | Host port the local Postgres service binds: `127.0.0.1:$port:5432`. Set `0` to let Docker allocate a free port. | live |
